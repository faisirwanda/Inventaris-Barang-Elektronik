...
include ("function/koneksi.php");
	...
	if (isset($_POST['submit'])){
		...
		if (strlen($kata_sandi) < 5) {
				echo "<script>alert('Password Harus 5 Karakter Atau Lebih');location.replace('index_user.php'); </script>";
		}
			else{
				if($_POST['kata_sandi']!=$konfirmasi_password) {
						echo "<script>alert('Password dan Konfirmasi Password Tidak Sama');location.replace('index_user.php.php');</script>";
				}
					else{
					$query = "UPDATE pengguna SET nama_pengguna='$nama_pengguna', kata_sandi='$kata_sandi', bagian='$bagian' WHERE nama_pengguna='$nama_pengguna'";
					 ...
					 if(mysqli_error($koneksi)) {
						...
						echo "<script>alert('Nama Pengguna dan Kata Sandi Berhasil diubah Nama Pengguna : $nama_pengguna dengan Kata Sandi : $kata_sandi dan Bagian : $bagian ' );location.replace('index_user.php');</script>";
					 }	
					}
			}
